...
## Testing Guidelines
There is currently no automated test suite or coverage gate in `package.json`. For changes, run `pnpm build` as the minimum verification step. For UI or terminal behavior, also validate with `pnpm start`; use `pnpm capture` when a static render is useful for review. When adding tests later, place them beside the relevant module or under a dedicated `tests/` directory and name them after the unit under test, for example `settings-store.test.ts`.
